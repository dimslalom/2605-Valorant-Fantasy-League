...
test('human matches wait on a timer that the host can advance early', () => {
  const state = lobbyWithPlayers(2);
  seedRosters(state);
  ...
  state.pendingTransition = { type: 'play_match', matchId: firstHuman.id, deadlineAt: 10_000 };

  assert.equal(firstHuman.winner, null);
  assert.equal(nextAlarmAt(state), 10_000);
  run(state, 'advance_early', 'p1', {}, cards, 100);
...
test('human match timer starts the match automatically at its deadline', () => {
  const state = lobbyWithPlayers(2);
  seedRosters(state);
  state.draft = { seatOrder: state.competitors.map(player => player.id) };
  state.season = {
    cycle: 0, eventIndex: 0, events: [{ kind: 'masters', city: 'London', label: 'Masters London' }],
    standings: Object.fromEntries(state.competitors.map(player => [player.id, {
      competitorId: player.id, squadName: player.squadName,
      titles: 0, matchWins: 0, mapsWon: 0, mapsLost: 0, score: 0,
    }])),
  ...
  state.tournament = { ...buildMultiplayerBracket(state, cards, 'masters'), meta: state.season.events[0], championId: null };
  const match = state.tournament.rounds[0].matches.find(item => item.humanInvolved);
  state.tournament.currentHumanMatchId = match.id;
  state.phase = 'match_ready';
  state.pendingTransition = { type: 'play_match', matchId: match.id, deadlineAt: 10_000 };

  advanceDeadlines(state, cards, 10_000);
...
test('v1 lobby migration strips the removed shop economy and adds the shared world', () => {
  const state = lobbyWithPlayers(2, { gameLength: 'endless' });
  state.schemaVersion = 1;
  state.rulesVersion = 1;
  state.shop = { packs: {}, deadlineAt: 1 };
  for (const player of state.competitors) {
    player.lives = 3; player.credits = 500; player.fatigue = 10;
    player.boosts = { x: [] }; player.teamChemBonus = 4;
  }
  const migrated = migrateLobbyState(state);

  assert.equal(migrated.schemaVersion, 2);
  // The V2 economy is gone, not merely emptied.
  assert.equal('shop' in migrated, false);
  for (const player of migrated.competitors) {
    for (const key of ['lives', 'credits', 'fatigue', 'boosts', 'teamChemBonus']) {
      assert.equal(key in player, false, `${key} should be stripped`);
    }
  }
  // ...and the shared living world is in place for an in-flight lobby.
  assert.deepEqual(migrated.world, { dev: {}, signedIds: {}, feed: [], year: 0 });
  assert.ok(migrated.competitors.every(player => player.bonds && typeof player.bonds === 'object'));
});
